...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float expected_w = CalcItemWidth() * 0.5f; // Change the width calculation to be half of the original size
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x * 0.5f, frame_bb.Min.y + style.FramePadding.y), label); // Adjust the label position
    ...
    const float combo_box_width = expected_w; // Store the modified width for rendering
    ...
    bool ret = Begin(name, NULL, window_flags); // Begin the modified smaller popup window
    PopStyleVar();
    if (!ret)
    {
        EndPopup();
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth() * 0.5f; // Reduce the width by half
